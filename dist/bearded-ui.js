...
var oe = /*@__PURE__*/ u({
	...
	setup(e) {
		...
		return (t, n) => (y(), i(w(s.value), p({
			...
			_: 3
		}, 16, ["aria-pressed", "class"]));
	}
}), [["__scopeId", "data-v-4a0e18d6"]]), z = 8, ce = 120, le = {
	bottom: "top",
	left: "right",
	right: "left",
	top: "bottom"
...
function B(e, t, n = {}) {
	...
	let u = {
		...
		top: l.top
	}, f = s === "top" || s === "bottom", p = le[s], m = (f ? u.height : u.width) + o + z, h = d[s] >= m || d[s] >= d[p] ? s : p, g = Math.max(d[h] - o - z, ce), _ = Math.min(f ? u.height : u.width, g), v = h === "top" || h === "left" ? (f ? l.top : l.left) - o - _ : (f ? l.bottom : l.right) + o, y = f ? l.left : l.top, b = f ? l.right : l.bottom, x = f ? u.width : u.height, S = f ? window.innerWidth : window.innerHeight, C = r === "end" ? b - x : r === "center" ? y + (b - y - x) / 2 : y, w = Math.min(Math.max(z, C), Math.max(z, S - x - z)), T = f ? v : w, E = f ? w : v;
...
function tt() {
	Y || typeof document > "u" || (Y = !0, q.value = et(), re(() => {
...
var Z = [
...
], nt = { class: "bd-theme-picker" }, rt = { class: "bd-theme-picker-inputs" }, it = { class: "bd-field" }, at = { class: "bd-field-label bd-font-bold" }, ot = { class: "bd-theme-picker-control" }, st = ["value"], ct = { class: "bd-field" }, lt = { class: "bd-field-label bd-font-bold" }, ut = { class: "bd-theme-picker-control" }, dt = ["value"], ft = { class: "bd-theme-picker-grid" }, pt = ["aria-pressed", "onClick"], mt = /*#__PURE__*/ P(/* @__PURE__ */ u({
	...
	setup(t) {
		...
		return (n, r) => (y(), o("div", nt, [s("div", rt, [s("label", it, [s("span", at, T(t.baseLabel), 1), s("span", ot, [s("input", {
			value: D(i).base,
			type: "color",
			onInput: r[0] ||= (e) => d("base", e)
		}, null, 40, st), s("code", null, T(D(i).base), 1)])]), s("label", ct, [s("span", lt, T(t.accentLabel), 1), s("span", ut, [s("input", {
			value: D(i).accent,
			type: "color",
			onInput: r[1] ||= (e) => d("accent", e)
		}, null, 40, dt), s("code", null, T(D(i).accent), 1)])])]), (y(!0), o(e, null, S(a.value, (t) => (y(), o("div", {
			key: t.group,
			class: "bd-theme-picker-group"
		}, [s("h3", null, T(t.group), 1), s("div", ft, [(y(!0), o(e, null, S(t.presets, (e) => (y(), o("button", {
			key: e.name,
			"aria-pressed": u(e),
		...
		}, [s("span", { style: g({ background: e.accent }) }, null, 4)], 4), c(" " + T(e.name), 1)], 8, pt))), 128))])]))), 128))]));
	}
}), [["__scopeId", "data-v-f56bcc72"]]), Q = x([]), ht = 0;
function $(e) {
	Q.value = Q.value.filter((t) => t.id !== e);
}
function gt(e, t = {}) {
	let { duration: n = 4e3, variant: r = "default" } = t, i = ht++;
...
var vt = ["onClick"], yt = /*#__PURE__*/ P(/* @__PURE__ */ u({
	__name: "BdToaster",
	props: { position: { default: "bottom-right" } },
	setup(r) {
		let a = r, { dismissToast: s, toasts: c } = _t();
...
//#endregion
export { se as BdBadge, R as BdButton, me as BdButtonGroup, ge as BdCard, be as BdCheckbox, Oe as BdConfirmDialog, U as BdDialog, Ne as BdDropdown, Ie as BdDropdownItem, He as BdInput, F as BdLoader, Ye as BdSelect, Ze as BdSlider, mt as BdThemePicker, yt as BdToaster, H as BdTooltip, G as bdDefaultPalette, Z as bdPresets, $ as dismissToast, W as isLightColor, gt as toast, X as useTheme, _t as useToast };
